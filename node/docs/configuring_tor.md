...
pearld provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, pearld supports a hybrid,
...
Configuring pearld as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the pearld command
line or in the pearld configuration file.  Typically the Tor proxy address will be
...
By default, pearld assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since pearld will act as the server.
...
```bash
./pearld --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
pearld provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the pearld command line or in the pearld
...
```bash
./pearld --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
pearld provides support for Tor stream isolation by using the `--torisolation`
flag.  This option requires --proxy or --onionproxy to be set.

### Command line example

```bash
./pearld --proxy=127.0.0.1:9050 --torisolation
```
