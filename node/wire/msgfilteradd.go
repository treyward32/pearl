...
func (msg *MsgFilterAdd) PrlDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	var err error
...
// This is part of the Message interface implementation.
func (msg *MsgFilterAdd) PrlEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	size := len(msg.Data)
	if size > MaxFilterAddDataSize {
		str := fmt.Sprintf("filteradd size too large for message "+
			"[size %v, max %v]", size, MaxFilterAddDataSize)
		return messageError("MsgFilterAdd.PrlEncode", str)
	}
