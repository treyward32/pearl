...
Oyster will not drop transaction history by itself, as this is something that
...
```bash
go run github.com/pearl-research-labs/pearl/wallet/cmd/dropwtxmgr@latest
```
...
```
$ dropwtxmgr
Database path: /home/username/.oyster/mainnet/wallet.db
Drop all Oyster transaction history? [y/N] y
...
```
$ dropwtxmgr --db ~/.oyster/testnet/wallet.db
Database path: /home/username/.oyster/testnet/wallet.db
Drop all Oyster transaction history? [y/N] y
Dropping wtxmgr namespace
```

After dropping transaction history, Oyster may be restarted and a full rescan
